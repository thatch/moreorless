...
def combined_diff(
    ...
    assert len(from_files) == len(from_filenames)
    assert len(to_files) == len(to_filenames)
    assert len(from_files) == 1 or len(to_files) == 1
    ...
    # print("MM", merge_mode)
    for block in _group(_contributions(from_files, to_files), context=context):
...
def _contributions(
    ...
    2*context runs of `((1,), (1, ...))`), and has the nice property that once
    ...
    if len(to_files) == 1:
        ...
        rest = from_files
        merge_mode = True
    elif len(from_files) == 1:
        common = from_files[0]
        rest = to_files
        merge_mode = False
    else:
        raise ValueError("At least one of from_files or to_files must be length one.")
...
def _group(lines: List[Line], context: int = 3) -> Iterable[Union[Hunk, Snip]]:
